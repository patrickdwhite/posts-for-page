...
function sc_posts_for_page($atts, $content = null){
  ...
  $options = array('length' => 50,
    ...
    'prev_text' => '&laquo; Newer Entries',
    'next_text' => 'Older Entries &raquo;',
    'show_full_post' => 'false'); 

  foreach( $options as $optname => $defval) {
    $_opts[$optname] = value_or_default($atts, $optname, $defval);
  }
    ...
    foreach( $children as $child ) {
      ...
      if($_opts['show_full_post'] != "true")
      {
        ...
        {   
          ...
          $words = array_slice($words, 0, $_opts['length']);
      ...
      {
        if( ($_opts['hide_images'] != 'true') && ($_opts['show_full_post'] != "true"))
        {
          $output .= $imageSrc;
        }
        $output .= $content;
        if( ($_opts['hide_read_more'] != 'true') && ($_opts['show_full_post'] != "true"))
        {    
          $output .= "<a href='$link' class='pfpReadMore'>" . $_opts['read_more'] . "</a>";
        }
      }
  ...
  {
    // show prev
    $output .= "<span class='pfpNav'>" . get_previous_posts_link($_opts['prev_text']) . "</span>";
  }
  if($_opts['cur_page'] <  $page_links_total)
  {
    // show next
    $output .= "<span class='pfpNav'>" . get_next_posts_link($_opts['prev_text']) . "</span>";
  }
